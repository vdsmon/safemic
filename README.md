...
SafeMic is a system-wide microphone mute for macOS. Toggle every input device at once with a global shortcut or from the menu bar, and get a volume-OSD-style bezel as visual confirmation.
...
Mute with <kbd>Cmd</kbd> <kbd>Shift</kbd> <kbd>A</kbd> (configurable) or from the menu bar dropdown, which has **Mute**, **Settings…**, **About**, and **Quit**. The menu bar icon always shows the current state — including when something else changes the mute state behind SafeMic's back.

## Features

- Mutes every CoreAudio input device via the native mute control. Devices without one fall back to input-volume-zero (restored on unmute); devices exposing neither control are skipped.
- Polls for hot-plugged devices (USB, Bluetooth) and re-asserts mute on anything that appears while the mic should be off.
- Detects devices that accept a mute command but never actually change state (some virtual devices, e.g. Microsoft Teams Audio) and excludes them so they can't wedge the mute state of your real microphones.
- Global shortcut with click-to-record capture in Settings and validation against macOS-reserved combos.
- Confirmation bezel styled like the system volume OSD: red slashed mic when muted, appears on whichever monitor your cursor is on, ignores mouse events, and is excluded from screenshots and recordings.
- Menu bar icon adapts to the menu bar's light/dark appearance.
- Launch at login.
- App appearance follows the system, or force Light/Dark from Settings.

## Settings

Open **Settings…** from the menu bar icon. Every change applies and saves immediately — there is no Save button. The window edits the mute shortcut (click the shortcut chip to record a new one), launch at login, how long the confirmation bezel stays on screen, and the app appearance (System, Light, or Dark).

The same settings live in `~/Library/Application Support/safemic/settings.json`, which you can also edit by hand — SafeMic picks up file changes while running. Missing keys fall back to defaults, so the file may be partial.
...
| `popup_duration_ms` | `number` | `1000` | How long the confirmation bezel stays visible after a mute/unmute event, in milliseconds. `0` hides the bezel entirely; the menu bar icon still updates. |
...
- Mutes CoreAudio-controllable devices only; devices with neither a mute nor an input-volume control are skipped.
- Devices that misreport their mute state are excluded from mute management rather than controlled.
- Polling can leave brief mute gaps on newly connected devices.
- Drivers can lie; use hardware mute, unplug, or macOS microphone permissions for high assurance.
...
On first launch, grant SafeMic **Microphone** access (CoreAudio control) and **Input Monitoring** (global shortcut) when prompted. The global shortcut also needs **Accessibility**, which macOS does not prompt for — add SafeMic manually under System Settings → Privacy & Security → Accessibility.
